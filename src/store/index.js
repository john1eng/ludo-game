...
const ludoSlice = createSlice({
  ...
  reducers: {
    ...
    turnChange (state) {
      if (state.pieces[state.turnColor].home === 4 || state.dice === 6) { return }
    ...
    moveToBoard (state, action) {
      ...
      if (state.pieces[color].start === 0) { return }
    ...
    movePlayer (state, action) {
      ...
      function movePieceFromHome (num) {
        ...
        if (num === 6) { state.pieces[color].home = state.pieces[color].home + 1 }
      }
